...
def cmd_start(message):
    ...
    bot.send_message(message.from_user.id, text , reply_markup=keyboardmain, parse_mode='html')
...
def аccept_message(message):
    ...
    if message.text == "📁 Каталог":
        ...
        bot.send_message(message.from_user.id, "Походу меня только что исправили, но сейчас уже всё ок")
        user_category = ["1"]
        cmd_start(message)
        # do_order(message)
...
def edit_profile(message):
    ...
    data = {"ФИО":["Введите свои ФИО"], "Адрес": ["Введите адрес"], "Город":["Введите название города"], "Телефон":["Введите номер телефона"]}
    if message.text in list(data.keys()):
        global last_bot_text
        last_bot_text = data[message.text][0]
        markup = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
        markup.add(types.KeyboardButton("Назад"))
        bot.send_message(message.chat.id, data[message.text][0], reply_markup=markup)
    ...
    # if message.text in ["Редактировать профиль","Назад"]:
        ...
        markup.add("👩‍🦽 Профиль")
        bot.send_message(message.chat.id, "Выберете что изменить", parse_mode="html", reply_markup=markup)
...
def show_profile(message):
    ...
    text = f"""ФИО: {fio}\nНомер : {phone_number}\nГород : {city}\nАдрес : {address}"""
    keyboardmain = types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    keyboardmain.add(types.KeyboardButton(text="Редактировать профиль"), types.KeyboardButton(text="⏺В главное меню"))
    bot.send_message(message.chat.id, text, parse_mode="html", reply_markup=keyboardmain)
...
def show_basket(message):
    ...
    if show_product_id >= min_id and show_product_id <= max_id:
        ...
        name_cat_pr = name_parents_category_for_this_product[0][0] + " -> " + str(about_product[0][1])

        markup = types.InlineKeyboardMarkup(row_width=1)
        clear = telebot.types.InlineKeyboardButton('✖️', callback_data='basket_clear')
        remove = telebot.types.InlineKeyboardButton('➖', callback_data='basket_remove')
        ...
        summ = 0
        for i in basket:
            summ += i[2] * select_from_shop_db("*", "product", f"_id = {i[1]}")[0][3]
        
        complite = telebot.types.InlineKeyboardButton(f'Сделать заказ на {summ} ₽', callback_data='complite')
        additionally = telebot.types.InlineKeyboardButton('Добавить ещё товар', callback_data='additionally')
...
def next_category(message):
    ...
    if categories(use_user_category) != []:
        ...
        if message.text in list_fo_check:
            use_user_category = int(sum_element_in_list(user_category)[-1])
            user_category.append(str(return_one_value(select_from_shop_db("_id", "categories" , "title = '{}' AND parents_categories = '{}'".format(message.text, use_user_category)))))
            do_order(message)
        else:
...
def show_product(message):
    if message.text != "< Назад" and message.text !=  "⏺В главное меню" and message.text !=  "Заполнить профиль":
        ...
        id_product = return_one_value(select_from_shop_db("_id", "product" , "title = '{}' AND id_categories = '{}'".format(message.text, int(sum_element_in_list(user_category)[-1]))))
        amount_product = return_one_value(select_from_user_basket_db("amount", "user_"+str(message.from_user.id), f"""id_product = {id_product}"""))

        if amount_product != None:
            pass
        else:
            amount_product = 0
        caption = """Название: {}\nЦена: {} ₽\nОписание:\n{}\n\nКол-во в корзине: {}""".format(all_about_product[1], all_about_product[3], all_about_product[2], amount_product)

        markup = types.InlineKeyboardMarkup(row_width=1)
        item3 = types.InlineKeyboardButton("Добавить в корзину (+1)", callback_data="add")
        item2 = types.InlineKeyboardButton("🛍 Корзина", callback_data="go to basket")
...
def data(call):
    if call.message:
        ...
        if call.data == "add":
            ...
            item2 = types.InlineKeyboardButton("🛍 Корзина", callback_data="go to basket")
            markup.add(item2, item3)
            id_product = return_one_value(select_from_shop_db("_id", "product" , "title = '{}' AND id_categories = '{}'".format(title, int(sum_element_in_list(user_category)[-1]))))
            amount = return_one_value(select_from_user_basket_db("amount", "user_"+str(call.message.chat.id), f"""id_product = {id_product}"""))
            all_about_product = []
            for i in select_from_shop_db("*", "product", "title = '{}' AND id_categories = '{}'".format(title, int(sum_element_in_list(user_category)[-1]))):
                all_about_product = list(i)
            text = """Название: {}\nЦена: {} ₽\nОписание:\n{}\n\nКол-во в корзине: {}""".format(all_about_product[1], all_about_product[3], all_about_product[2], amount)
            ...
            if basket == []:
                ...
                bot.send_message(call.message.chat.id, "В вашей корзине **пусто** !", reply_markup=keyboardmain)
                return
            if "add" in call.data:
            ...
            if show_product_id >= min_id and show_product_id <= max_id:
                ...
                if caption != call.message.caption:
                    bot.edit_message_media(media=telebot.types.InputMedia(type='photo', media=new_photo, caption = caption), chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
            ...
            if None not in select_from_clients_db("*", "clients",whereis=f"user_id = {call.message.chat.id}")[0][2:]:
                ...
                всего = 0
                for i in basket:
                    about_product = select_from_shop_db("*", "product", f"_id = {i[1]}")
                    name_parents_category_product = select_from_shop_db("title", whereis=f"_id = {about_product[0][5]}")
                ...
                bot.send_message(call.message.chat.id, "Чтобы сделать заказ заполниете профиль в главном меню", reply_markup=markup)

        elif call.data == "additionally":
            keyboardmain = types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
            first_button = types.KeyboardButton(text="📁 Каталог")
            keyboardmain.add(first_button)
            bot.send_message(call.message.chat.id,"Тогда давай сначала", reply_markup=keyboardmain)


def do_order(message):
    global user_category
    try:
        use_user_category = int(sum_element_in_list(sum_element_in_list(user_category)[-1]))
        ...
        if categories(use_user_category) != []:
            ...
            bot.send_message(message.from_user.id, "Отлично, нажимай на нужную категорию", reply_markup=markup)
            ...
            markup.add("⏺В главное меню", "< Назад")
            # markup.add("< Назад")
            bot.send_message(message.from_user.id, "Отлично, выбери продукт", reply_markup=markup)
            bot.register_next_step_handler(message, show_product)
    except IndexError:
        user_category = ["1"]
        cmd_start(message)
